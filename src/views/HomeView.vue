...
const loadPokemonTypes = async () => {
	pokemonTypes.value = [];
	pokemons.value.forEach((pokemon: any) => {
		pokemon.types.forEach((type: any) => {
			if (!pokemonTypes.value.some((t: any) => t.name === type.type.name)) {
				pokemonTypes.value.push(type.type);
			}
		});
	});
}
const fetchPokemons = async () => {
	try {
		...
		else url += "?limit=" + (pokemons.value.length - offset.value) + "&offset=" + offset.value;

		const response = await axios.get(url);

		response.data.results.forEach(async (element: any, index: number) => {
			const pokemon = await axios.get(element.url);
			const pokemonObject: any = pokemon.data;

			pokemons.value.push(pokemonObject);
...
const changeLoadAmount = (event: any) => {
	const requestedAmount: number = parseInt(event.target.getAttribute('data'), 10)
			...
			<div class="w-full flex flex-col lg:flex-row justify-between items-center py-8" >
				...
				<div class="flex justify-center lg:justify-between items-center gap-2 mt-4 lg:mt-0 flex-wrap">
					<button 
						...
						:data="value.data"
						:class="[value.data === loadAmount ? 
							'bg-orange-300 text-slate-800': 
							'bg-slate-200 text-slate-600']"	
						@click="changeLoadAmount"			
					>
					{{ value.data === loadAmount ? "Showing" : "Show" }} {{ value.display }}
					</button>
				</div>
			</div>
